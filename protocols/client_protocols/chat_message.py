...
class ChatCommand:
    ...
    def match(message: bytes, socket_type: str, client_data) -> bool:
        ...
        return socket_type == ChatCommand.SOCKET_TYPE and message[0] == ChatCommand.PACKET_ID[0] and len(
            message) > ChatCommand.HEADER_SIZE and client_data["logged_in"]

    @staticmethod
    def get_answer(headers: bytes, message, client_data, clients) -> str:
        """
        ...
        for data in clients:
            if data["ID"] != client_data["ID"] and data["logged_in"]:
                data["message_queue"].put(msg_)
        return msg_
    ...
    def get_headers(message: bytes):
        ...
        return packet_id, message_color
    ...
    def format_message(message, player_id, color=(0, 0, 0)):
        ...
        return f"{ChatCommand.PACKET_ID.decode()}{''.join([x.to_bytes(1, 'little').decode() for x in color])}" \
               f"{player_id.to_bytes(1, 'little').decode()}{message}"
